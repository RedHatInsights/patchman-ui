...
export function getSeverityById(id) {
  return advisorySeverities.find((item) => item.value === id) || advisorySeverities[0];
}
  ...
  const buildChips = (filters, category) => {
    if (parsers[category]) {
      ...
      return [].concat(filters[category]).map((filterValue) => {
        const match = values.find((item) => item.value.toString() === filterValue.toString());
        return {
          name: match.label,
          value: filterValue,
          id: match.value,
...
export const createOSColumn = ({ osName, rhsm }) =>
  !rhsm ? (
    ...
    <Tooltip content={intl.formatMessage(messages.textLockVersionTooltip, { lockedVersion: rhsm })}>
...
export const mapGlobalFilters = (tags, SIDs, workloads = {}) => {
  ...
  globalFilterConfig.systemProfile = {
    ...
    ...(workloads?.['Microsoft SQL']?.isSelected && { mssql: { version: 'not_nil' } }),
