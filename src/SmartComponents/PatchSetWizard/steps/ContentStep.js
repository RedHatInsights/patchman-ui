...
const ContentStep = ({ patchSetID }) => {
  ...
  return (
    <Stack hasGutter>
      <StackItem>
        <Content>
          <Content component='h2'>{intl.formatMessage(messages.templateContentStepTitle)}</Content>
        </Content>
      </StackItem>
      <StackItem>{intl.formatMessage(messages.templateContentStepText)}</StackItem>

      <StackItem>{formOptions.renderForm(toDateComponent)}</StackItem>

      <StackItem>
        <ExpandableSection
          toggleText={intl.formatMessage(messages.templateContentStepExpandableTitle)}
        >
